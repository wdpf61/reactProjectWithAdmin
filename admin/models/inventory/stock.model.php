...
class Stock extends Model implements JsonSerializable{
	...
	static function stock_report(){
		global $db,$tx;
		$result =$db->query("SELECT s.id,p.name product,sum(s.qty) qty FROM {$tx}stocks s, {$tx}products p where p.id = s.product_id and  date(s.updated_at) BETWEEN '2025-01-01' and '2025-10-20' GROUP by product_id");
		$data=[];
		while($stock=$result->fetch_object()){
			$data[]=$stock;
		}
	...
	static function stock_report_by_date($from, $to){
		global $db,$tx;
		$result =$db->query("SELECT s.id,p.name product,sum(s.qty) qty FROM {$tx}stocks s, {$tx}products p where p.id = s.product_id and  date(s.updated_at) BETWEEN '$from' and '$to' GROUP by product_id");
		$data=[];
		while($stock=$result->fetch_object()){
			$data[]=$stock;
		}
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($stock=$result->fetch_object()){
			...
			$html.="<tr><td>$stock->id</td><td>$stock->product_id</td><td>$stock->qty</td><td>$stock->transaction_type_id</td><td>$stock->remark</td><td>$stock->created_at</td><td>$stock->warehouse_id</td><td>$stock->updated_at</td><td>$stock->lot_id</td> $action_buttons</tr>";
		}
		$html.="</table>";
		$html.= pagination($page,$total_pages);
		return $html;
	}
	static function html_table_report($page = 1,$perpage = 10,$criteria="",$action=true){
		global $db,$tx,$base_url;
		$count_result =$db->query("select count(*) total from {$tx}stocks $criteria ");
		list($total_rows)=$count_result->fetch_row();
		$total_pages = ceil($total_rows /$perpage);
